...
\hypersetup{unicode=true,
            pdftitle={Applying web scrapping technics in collecting GRE analytical writing topics},
            pdfauthor={Guo Shiyuan},
            pdfborder={0 0 0},
            breaklinks=true}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## For issue, find the index with sentense start with "claim"}
\NormalTok{start_claim <-}\StringTok{ }\KeywordTok{unname}\NormalTok{(}\KeywordTok{which}\NormalTok{(}\KeywordTok{sapply}\NormalTok{(issue, substr, }\DataTypeTok{start =} \DecValTok{1}\NormalTok{, }\DataTypeTok{stop =} \DecValTok{5}\NormalTok{ ) }\OperatorTok{==}\StringTok{ "Claim"}\NormalTok{))}
\NormalTok{## paste start with "claim" item with each of the following items (started with "Reason")}
\NormalTok{issue[start_claim] <-}\StringTok{ }\KeywordTok{paste}\NormalTok{(issue[start_claim], }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{,issue[start_claim}\OperatorTok{+}\DecValTok{1}\NormalTok{]) }
\NormalTok{## delete item start with "Reason"}
\NormalTok{issue <-}\StringTok{ }\NormalTok{issue[}\OperatorTok{-}\NormalTok{(start_claim }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{)]}
\NormalTok{## now odd is prompt and even is question }
\NormalTok{issue_tbl <-}\StringTok{ }\KeywordTok{tbl_df}\NormalTok{(}\KeywordTok{matrix}\NormalTok{(issue, }\DataTypeTok{ncol =} \DecValTok{2}\NormalTok{, }\DataTypeTok{byrow =}\NormalTok{ T)) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{mutate}\NormalTok{(., }\DataTypeTok{qn_no =} \KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{factor}\NormalTok{(V2))) }\CommentTok{# number the question }
\KeywordTok{colnames}\NormalTok{(issue_tbl) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"background_info"}\NormalTok{, }\StringTok{"question"}\NormalTok{, }\StringTok{"qn_no"}\NormalTok{)}

\NormalTok{######################################################################}
\NormalTok{## for argument, double-line promt has no single/simple pattern, we identied/indexed questions. }
\NormalTok{start_response <-}\StringTok{ }\KeywordTok{unname}\NormalTok{(}\KeywordTok{which}\NormalTok{(}\KeywordTok{sapply}\NormalTok{(argument, substr, }\DataTypeTok{start =} \DecValTok{1}\NormalTok{, }\DataTypeTok{stop =} \DecValTok{16}\NormalTok{) }\OperatorTok{==}\StringTok{ "Write a response"}\NormalTok{))}
\NormalTok{## creating interval between each question to next. }
\NormalTok{interval <-}\StringTok{ }\KeywordTok{findInterval}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{493}\NormalTok{, start_response, }\DataTypeTok{rightmost.closed =}\NormalTok{ T, }\DataTypeTok{left.open =}\NormalTok{ T)}
\NormalTok{interval[}\DecValTok{2}\NormalTok{] <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{) ## a minor adjust of the first interval. }
\NormalTok{## split the prompts out, aggregate paste neighbour with sample interval number. }
\NormalTok{interval_tbl <-}\StringTok{ }\KeywordTok{tbl_df}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(argument, interval)) }\OperatorTok{%>%}
...
\NormalTok{bg_tbl <-}\StringTok{ }\KeywordTok{aggregate}\NormalTok{(argument}\OperatorTok{~}\NormalTok{interval, interval_tbl, paste, }\DataTypeTok{collapse =} \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{    }\NormalTok{tbl_df}
\NormalTok{## split the questions out. }
\NormalTok{qn_tbl <-}\StringTok{ }\KeywordTok{tbl_df}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(argument, interval)) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{group_by}\NormalTok{(., interval) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{filter}\NormalTok{(., }\KeywordTok{row_number}\NormalTok{() }\OperatorTok{==}\StringTok{ }\KeywordTok{n}\NormalTok{()) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{ungroup}\NormalTok{()}
\NormalTok{## joining prompts and questions. }
\NormalTok{argument_tbl <-}\StringTok{ }\KeywordTok{inner_join}\NormalTok{(bg_tbl, qn_tbl, }\DataTypeTok{by =} \KeywordTok{c}\NormalTok{(}\StringTok{"interval"}\NormalTok{ =}\StringTok{ "interval"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{select}\NormalTok{(., }\OperatorTok{-}\NormalTok{interval) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{mutate}\NormalTok{(., }\DataTypeTok{qn_no =} \KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{factor}\NormalTok{(argument.y)))}
\KeywordTok{colnames}\NormalTok{(argument_tbl) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"background_info"}\NormalTok{, }\StringTok{"question"}\NormalTok{, }\StringTok{"qn_no"}\NormalTok{)}

\NormalTok{######################################################################}
\NormalTok{## writing in excel}
\CommentTok{# wb <- createWorkbook()}
\CommentTok{# }
\CommentTok{# sn <- "argument"}
\CommentTok{# addWorksheet(wb = wb, sheetName = sn)}
\CommentTok{# writeData(wb = wb, sheet = sn, x = argument_tbl, borders = "n")}
\CommentTok{# }
\CommentTok{# sn <- "issue"}
\CommentTok{# addWorksheet(wb = wb, sheetName = sn)}
\CommentTok{# }
\CommentTok{# writeData(wb = wb, sheet = sn, x = issue_tbl, borders = "n")}
\CommentTok{# saveWorkbook(wb, file = "./static/GRE_AW.xlsx", overwrite = TRUE)}
\end{Highlighting}
\end{Shaded}
...
``Palean'' basket in Lithos, an ancient village across the Brim River
from Palea. The Brim River is very deep and broad, and so the ancient
...
journal. ``A recent study of eighteen rhesus monkeys provides clues as
to the effects of birth order on an individual's levels of stimulation.
The study showed that in stimulating situations (such as an encounter
with an unfamiliar monkey), firstborn infant monkeys produce up to twice
as much of the hormone cortisol, which primes the body for increased
activity levels, as do their younger siblings. Firstborn humans also
produce relatively high levels of cortisol in stimulating situations
(such as the return of a parent after an absence). The study also found
that during pregnancy, first-time mother monkeys had higher levels of
